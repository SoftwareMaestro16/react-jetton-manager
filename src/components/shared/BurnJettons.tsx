...
export function BurnJettons({ jettons, loading }: Props) {
  const [transactions, setTransactions] = useState([{ token: "", amount: "" }]);
  ...
  const addTransaction = () => {
    if (transactions.length < 3) {
      setTransactions([...transactions, { token: "", amount: "" }]);
    }
  };
  ...
  const onBurnTx = async () => {
    ...
    try {
      ...
      if (messages.length === 0) {
        console.warn("No valid burn transactions");
      ...
      try {
        ...
        console.error("Burn failed to confirm:", e);
      }
  ...
  return loading ? (
    ...
    <>
      <div className="p-4 bg-[#0e0e0e] border border-amber-600 rounded-lg flex flex-col max-h-[55vh]">
        <h3 className="text-white text-lg font-semibold mb-3">Burn Jettons</h3>
        ...
        >
          {txInProgress ? "Burning..." : "Burn Jettons"}
        </button>
      </div>
